...
import ru.denull.wire.stub.tl.upload.GetFile;
import ru.denull.wire.stub.tl.upload.SaveFilePart;
...
public class FileManager {
  ...
  */
        ...
        public void load() {
            manager.service.connectAndPrepare(dc_id, false, false, false, new AuthCallback() {
                ...
                public void done(Server server, byte[] auth_key) {
                    server.call(new GetFile(location, loaded, CHUNK_SIZE), new Server.RPCCallback<ru.denull.wire.stub.tl.upload.File>() {
                        public void done(ru.denull.wire.stub.tl.upload.File result) {
                            append(result.bytes);
    ...
    public int getState(Object location) {
        ...
        } else if (location instanceof Video) {
