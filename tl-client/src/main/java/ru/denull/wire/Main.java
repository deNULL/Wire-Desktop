...
public class Main implements OnUpdateListener, TypingCallback {
    ...
    private void initialize() {
        ...
        dialogList.addListSelectionListener(new ListSelectionListener() {
            ...
            //dialogList.clearSelection();*/
                    } else if (dialogsBtn.isSelected()) {
                        if (service.dialogManager.isEmpty()) return;

                        ru.denull.wire.stub.tl.Dialog dialog = service.dialogManager.all.get(dialogList.getSelectedIndex());
                        selectDialog(dialog);
                    } else if (contactsBtn.isSelected()) {
                        if (contactListModel.isEmpty()) return;
        ...
        titleActionBtn.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (currentPeer != null) {
                    if (currentPeer instanceof InputPeerChat) {
                        ru.denull.wire.stub.tl.TChat chat = service.chatManager.get(currentPeer.chat_id);
                        if (!(chat instanceof ChatForbidden) && chat.left) {
                            service.mainServer.call(new ru.denull.wire.stub.tl.messages.AddChatUser(currentPeer.chat_id, new InputUserSelf(), 20), new RPCCallback<ru.denull.wire.stub.tl.messages.TMessage>() {
                                public void done(ru.denull.wire.stub.tl.messages.TMessage result) {
                        ...
                        } else if (JOptionPane.showOptionDialog(frame, "Покинув чат, вы не сможете вернуться в него обратно.", "Подтвердите действие", JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE, null, new String[]{"Отмена", "Выйти из чата"}, "Отмена") == 1) {
                            //System.out.println("leaving");
                            titleActionBtn.setVisible(false);
                            sendPanel.setVisible(false);

                            service.mainServer.call(new ru.denull.wire.stub.tl.messages.DeleteChatUser(currentPeer.chat_id, new InputUserSelf()), new RPCCallback<ru.denull.wire.stub.tl.messages.TMessage>() {
                                public void done(ru.denull.wire.stub.tl.messages.TMessage result) {
                                    service.chatManager.store(result.chats);
                                    service.userManager.store(result.users);
                                    service.messageManager.store(result.message);
    ...
    /*chatMemberList.addListSelectionListener(new ListSelectionListener() {
      public void valueChanged(ListSelectionEvent e) {
        ...
        service.mainServer.call(new ru.denull.wire.stub.tl.account.UpdateStatus(false));
        Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
            public void run() {
                service.mainServer.call(new ru.denull.wire.stub.tl.account.UpdateStatus(true));
            }
        }));
    }
    ...
    protected void editChat() {
        ...
        if (currentChat != null) {
            chatMemberModel.add(currentChat.full_chat.participants.participants);
        }
        ...
        memberListRenderer.actionListener = new ContactListRenderer.ContactActionListener() {
            public void onContactSelected(final int user_id, int index) {
                if (JOptionPane.showOptionDialog(frame, "Вы уверены, что хотите исключить участника из чата?", "Подтвердите действие", JOptionPane.OK_CANCEL_OPTION, JOptionPane.WARNING_MESSAGE, null, new String[]{"Отмена", "Исключить из чата"}, "Отмена") == 1) {
                    ...
                    TUser user = service.userManager.get(user_id);
                    service.mainServer.call(new ru.denull.wire.stub.tl.messages.DeleteChatUser(currentPeer.chat_id, user instanceof UserContact ? new InputUserContact(user_id) : new InputUserForeign(user_id, user.access_hash)), new RPCCallback<ru.denull.wire.stub.tl.messages.TMessage>() {
                        public void done(ru.denull.wire.stub.tl.messages.TMessage result) {
                            service.chatManager.store(result.chats);
                            service.userManager.store(result.users);
                            service.messageManager.store(result.message);
                            ...
                            service.mainServer.call(new GetFullChat(currentPeer.chat_id), new Server.RPCCallback<ru.denull.wire.stub.tl.messages.ChatFull>() {
                                public void done(ru.denull.wire.stub.tl.messages.ChatFull result) {
                                    if (result.full_chat.id == currentPeer.chat_id) {
                                        service.userManager.store(result.users);
                                        service.chatManager.store(result.chats);
                                        currentChat = (ChatFull) result;
                                        updateStatus();
                                    }
                                }
            ...
            public boolean isVisible(int user_id, int index) {
                if (currentChat == null || currentChat.full_chat.participants == null) return false;

                if (currentChat.full_chat.participants.admin_id == service.me.id || user_id == service.me.id) {
                    return true;
                }

                for (TChatParticipant part : currentChat.full_chat.participants.participants) {
        ...
        contactListRenderer.actionListener = new
                ContactListRenderer.ContactActionListener() {
                    public void onContactSelected(final int user_id, int index) {
                        chatMemberModel.add(user_id, true);

                        TUser user = service.userManager.get(user_id);
                        service.mainServer.call(new ru.denull.wire.stub.tl.messages.AddChatUser(currentPeer.chat_id, user instanceof UserContact ? new InputUserContact(user_id) : new InputUserForeign(user_id, user.access_hash), 10), new RPCCallback<ru.denull.wire.stub.tl.messages.TMessage>() {
                            public void done(ru.denull.wire.stub.tl.messages.TMessage result) {
                                service.chatManager.store(result.chats);
                                service.userManager.store(result.users);
                                service.messageManager.store(result.message);
    ...
    protected void saveChat() {
        ...
        service.mainServer.call(new EditChatTitle(currentPeer.chat_id, title), new RPCCallback<ru.denull.wire.stub.tl.messages.TMessage>() {

            public void done(final ru.denull.wire.stub.tl.messages.TMessage result) {
                service.chatManager.store(result.chats);
                service.userManager.store(result.users);
                service.messageManager.store(result.message);
                service.dialogManager.addMessage(result.message);
                ...
                if (vcf) {
                    ...
                    for (int i = 0; i < comp.length; i++) {
                        ...
                        if (head.equals("first name") || head.equals("given name")) {
                            fnameIndex = i;
                        } else if (head.equals("last name") || head.equals("family name")) {
                            lnameIndex = i;
                        } else if (head.equals("primary phone")) {
                            phoneIndex = i;
                        }
                    }
    ...
    private void addHint(final JTextField field, final String hint) {
        ...
        field.getDocument().addDocumentListener(new DocumentListener() {
            ...
            public void changedUpdate(final DocumentEvent e) {
                ...
                } else if (!field.getText().isEmpty() && field.getForeground().equals(Color.LIGHT_GRAY)) {
                    updating = true;
                    SwingUtilities.invokeLater(new Runnable() {
                        public void run() {
                            field.setForeground(Color.BLACK);
    ...
    public void sendMessage(String message, TInputPeer inputPeer) {
        ...
        final ru.denull.wire.stub.tl.TMessage newmsg = new ru.denull.wire.stub.tl.Message(random_id, service.me.id, peer, true, true, (int) (System.currentTimeMillis() / 1000), message, new MessageMediaEmpty());
//        newmsg.sending = true;
...
//                newmsg.failed = true;
    ...
    public void selectDialog(final TInputPeer peer) {
        ...
        if (peer instanceof InputPeerChat) {
            ...
            service.mainServer.call(new GetFullChat(chat_id), new Server.RPCCallback<ru.denull.wire.stub.tl.messages.ChatFull>() {
                public void done(ru.denull.wire.stub.tl.messages.ChatFull result) {
                    if (result.full_chat.id == currentPeer.chat_id) {
                        service.userManager.store(result.users);
                        service.chatManager.store(result.chats);
    ...
    public void selectDialog(ru.denull.wire.stub.tl.Dialog dialog) {
    ...
    public void restoreDialogSelection() {
        ...
        if (dialogsBtn.isSelected()) {
            ...
            for (int i = 0; i < service.dialogManager.all.size(); i++) {
                ru.denull.wire.stub.tl.Dialog d = service.dialogManager.all.get(i);
...
//                            futureMessage.failed = true;
    ...
    public void importContacts(ArrayList<String[]> contacts, boolean replace) {
        ...
        service.mainServer.call(new ru.denull.wire.stub.tl.contacts.ImportContacts(arr, replace), new Server.RPCCallback<ImportedContacts>() {
