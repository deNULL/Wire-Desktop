...
public class Server implements ReadTaskCallback {
        ...
        if (_queuedCalls.size() == 1) {
            ...
            if (call.encrypted) {
                if (auth.state == AuthState.FAILED) {
                    ...
                    return;
                } else if (auth.state != AuthState.COMPLETE) {
                    return;
                }
            }
    ...
    private void processMessage(TLObject message, long msg_id) throws Exception {
        ...
        } else if (message instanceof BadMsgNotification) {
        ...
        } else if (message instanceof BadServerSalt) {
        ...
        } else if (message instanceof TUpdates) {
