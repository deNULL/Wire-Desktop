...
import ru.denull.wire.stub.tl.auth.ImportAuthorization;
import ru.denull.wire.stub.tl.contacts.TForeignLink;
import ru.denull.wire.stub.tl.contacts.TMyLink;
import ru.denull.wire.stub.tl.help.GetConfig;
import ru.denull.wire.stub.tl.updates.*;
...
public class DataService {
    ...
    public void checkUpdates() {
        ...
        if (updates_pts < 0 || updates_date < 0) { // invalid (not initialized) state
            mainServer.call(new GetState(), new Server.RPCCallback<ru.denull.wire.stub.tl.updates.State>() {
            ...
            mainServer.call(new GetDifference(updates_pts, updates_date, 0), new Server.RPCCallback<TDifference>() {
                public void done(TDifference result) {
                    ...
                    } else if (result instanceof DifferenceSlice) {
    ...
    public void processUpdates(TUpdates updates) {
        ...
        if (updates instanceof UpdatesTooLong) {
            checkUpdates();
        } else if (updates instanceof UpdateShort) {
            ...
            processNewMessage(new ru.denull.wire.stub.tl.Message(upd.id, upd.from_id, new PeerUser(me == null ? 0 : me.id), false, true, upd.date, upd.message, new MessageMediaEmpty()), true);
            ...
            updates_date = Math.max(updates_date, upd.date);
            updates_seq = Math.max(updates_seq, upd.seq);
            processNewMessage(new ru.denull.wire.stub.tl.Message(upd.id, upd.from_id, new PeerChat(upd.chat_id), false, true, upd.date, upd.message, new MessageMediaEmpty()), true);
        } else if (updates instanceof Updates) {
        ...
        } else if (updates instanceof UpdatesCombined) {
